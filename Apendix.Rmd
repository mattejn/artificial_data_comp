...
Color embedded cells based on their type from ground truth. 
...
rnms = sapply(strsplit(rownames(received), split = '.', fixed = TRUE), function(x)
  (x[1]))
uq = unique(rnms)
...
qplot() + geom_point(aes(e_gt[, 1], e_gt[, 2], colour = rnms),
  ...
  ylab('') + xlab('') + theme_void()+scale_color_manual(breaks = uq,values = distinct20)+guides(color=guide_legend(ncol=2,title="Cell type"))
```
...
for(un in uq){
  ...
  rs=t.test(norm(log(as.matrix(ngd_err[coords_n,]))),norm(log(as.matrix(ngd_err[-coords_n,]))))
  if(rs$p.value<0.05)
    {print(paste0(un,' type is significantly different from other types combined with p-value of ',rs$p.value))}
}
...
for( i in 1:ncol(combinations))
{
  ...
  rs=t.test(norm(log(as.matrix(ngd_err[coords_a,]))),norm(log(as.matrix(ngd_err[coords_b,]))))
  if(rs$p.value<0.05)
    {print(paste0(combinations[1,i],' type is significantly different from ',combinations[2,i],' type with p-value of ',rs$p.value))}
}
