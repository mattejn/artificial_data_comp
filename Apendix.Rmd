...
```{r MSE}
cat(paste0('MSE for NGD: ', big$ngd$mse , '\n'))
cat(paste0('MSE for WOLS: ', big$ols$mse , '\n'))
cat(paste0('MSE for OLS: ', big$wols$mse , '\n'))
cat(paste0('MSE for NGD clamped to 0: ', big$zeroclamp_ngd$mse , '\n'))
cat(paste0('MSE for WOLS clamped to 0: ', big$zeroclamp_ols$mse , '\n'))
cat(paste0('MSE for OLS clamped to 0: ', big$zeroclamp_wols$mse , '\n'))
```


## Compare error distributions 

1. Compare error distribution shape and magnitude over different methods. (do)
...
plot(
  ...
  cex = .1,
  ylim = c(0, 50),
  main = paste0('NOUGAD MSE=', round(med$ngd$mse, 4), ' SD_mse=', round(sd(
    as.matrix(med$ngd$sq_err)
  ), 4)),
  xaxt = 'n'
)
...
for (i in 1:ncol(combos)) {
  ...
  p2 = qplot() + geom_point(aes(x_ngd, y_ngd, colour = aes_engd),
                            ...
                            alpha = 0.5) + scale_colour_gradientn(
                              ...
                              labels = c(' 0', ' 2', ' 5', '10 ', '20 ', '40 ', '60 '),
                              breaks = c(0, 2, 5, 10, 20, 40, 60)
                            ) + ggtitle('0 clamped NGD') + theme_cowplot() +
    ylab('') + xlab('') + scale_x_continuous(limits = c(mn_x, mx_x)) + scale_y_continuous(limits = c(mn_y, mx_y)) +
    theme_void()
  p3 = qplot() + geom_point(aes(x_wols, y_wols, colour = aes_ewols),
                            size = 1,
                            alpha = 0.5) + scale_colour_gradientn(
                              ...
                              labels = c(' 0', ' 2', ' 5', '10 ', '20 ', '40 ', '60 '),
                              breaks = c(0, 2, 5, 10, 20, 40, 60)
                            ) + ggtitle('0 clamped WOLS') + theme_cowplot() +
    ylab(combos[2, i]) + xlab(combos[1, i]) + scale_x_continuous(limits = c(mn_x, mx_x)) +
    scale_y_continuous(limits = c(mn_y, mx_y)) + theme_void()
  p4 = qplot() + geom_point(aes(x_ols, y_ols, colour = aes_eols),
                            size = 1,
                            alpha = 0.5) + scale_colour_gradientn(
                              ...
                              labels = c(' 0', ' 2', ' 5', '10 ', '20 ', '40 ', '60 '),
                              breaks = c(0, 2, 5, 10, 20, 40, 60)
                            ) + ggtitle('0 clamped OLS') + theme_cowplot() +
    ylab('') + xlab('') + scale_x_continuous(limits = c(mn_x, mx_x)) + scale_y_continuous(limits = c(mn_y, mx_y)) +
    theme_void()
  figure = ggarrange(
    p1,
...
p1 = ggplot()  + geom_segment(
  ...
  ggtitle('0 clamped NGD') + scale_x_discrete(labels = NULL, breaks = NULL) + labs(x = '', y =
                                                                                     '') + scale_y_discrete(labels = NULL, breaks = NULL) + theme_void()
p2 = ggplot()  + geom_segment(
  aes(
    x = e_gt[, 1],
  ...
  ggtitle('0 clamped WOLS') + scale_x_discrete(labels = NULL, breaks = NULL) + labs(x = '', y =
                                                                                      '') + scale_y_discrete(labels = NULL, breaks = NULL) + theme_void()
p3 = ggplot()  + geom_segment(aes(
  e_gt[, 1],
  y = e_gt[, 2],
...
colour = "red",
alpha = 0.3) + geom_point(aes(x = e_gt[, 1], y = e_gt[, 2]), size = 0.5, alpha =
                            0.4) + ggtitle('0 clamped OLS') + scale_x_discrete(labels = NULL, breaks = NULL) + labs(x = '', y =
                                                                                                                      '') + scale_y_discrete(labels = NULL, breaks = NULL) + theme_void()
...
qplot() + geom_point(aes(e_gt[, 1], e_gt[, 2], colour = rnms),
                     size = 0.4,
                     alpha = 0.6)  + ggtitle('Ground Truth') + theme_cowplot() +
...
#one-to-all
for (un in uq) {
  coords_n = which(rnms == un)
  rs = t.test(norm(big$ngd$mean_cell_err[coords_n]),
              norm(big$ngd$mean_cell_err[-coords_n]))
  if (rs$p.value < 0.05)
  {
    cat(
      paste0(
        un,
        ' type is significantly different from other types combined with p-value of ',
        round(rs$p.value, 8),
        '\n'
      )
    )
...
combinations = combn(uq, 2)
#one-to-one
for (i in 1:ncol(combinations))
{
  coords_a = which(rnms == combinations[1, i])
  coords_b = which(rnms == combinations[2, i])
  rs = t.test(norm(big$ngd$mean_cell_err[coords_a]),
              norm(big$ngd$mean_cell_err[coords_b]))
  if (rs$p.value < 0.05)
  {
    cat(
      paste0(
        combinations[1, i],
        ' type is significantly different from ',
        combinations[2, i],
        ' type with p-value of ',
        round(rs$p.value, 8),
        '\n'
      )
    )
